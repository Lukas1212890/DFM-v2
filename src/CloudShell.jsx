...
export default function CloudShell(){
 ...
 const loginWith=async(loginEmail,loginCode)=>request('/auth/code-login',{method:'POST',body:JSON.stringify({email:loginEmail,code:loginCode})},false);

 useEffect(()=>{(async()=>{try{
  const h=await request('/health',{},false);setInitialized(Boolean(h.initialized));
  const match=location.hash.match(/^#invite=(.+)$/);
  if(match&&!token&&!autoInviteDone.current){
   autoInviteDone.current=true;setAuth('checking');
   try{const data=JSON.parse(fromBase64Url(match[1]));if(!data.email||!data.code)throw new Error('Pozvánka nemá platný formát.');applySession(await loginWith(data.email,data.code));return;}
   catch(e){history.replaceState(null,'',location.pathname+location.search);setError(e.message||'Pozvánku se nepodařilo použít.');setAuth('login');return;}
  }
 ...
 const login=async e=>{e.preventDefault();setBusy(true);setError('');try{
  const r=initialized?await loginWith(email,code):await request('/auth/bootstrap',{method:'POST',body:JSON.stringify({email,name,code})},false);applySession(r);
 ...
 useEffect(()=>{if(auth!=='ready'||!token)return;const original=Storage.prototype.setItem;
  const initial=async()=>{if(!navigator.onLine){setStatus(localStorage.getItem(PENDING_KEY)?'Offline · čeká na synchronizaci':'Offline');return;}try{if(localStorage.getItem(PENDING_KEY)){await syncNow();return;}const r=await request('/state'),remote=r.data||EMPTY,local=JSON.parse(localStorage.getItem(STORAGE_KEY)||'null')||EMPTY;if(JSON.stringify(remote)!==JSON.stringify(local)){syncing.current=true;original.call(localStorage,STORAGE_KEY,JSON.stringify(remote));syncing.current=false;location.reload();}lastRemote.current=JSON.stringify(remote);setStatus('Online');}catch{setStatus('Offline');}};initial();
  Storage.prototype.setItem=function(k,v){original.call(this,k,v);if(this===localStorage&&k===STORAGE_KEY&&!syncing.current){original.call(localStorage,PENDING_KEY,'1');setStatus(navigator.onLine?'Čeká na synchronizaci':'Offline · čeká na synchronizaci');clearTimeout(saveTimer.current);saveTimer.current=setTimeout(()=>{try{pushState(JSON.parse(v));}catch{}},500);}};
  const online=()=>syncNow(),offline=()=>setStatus(localStorage.getItem(PENDING_KEY)?'Offline · čeká na synchronizaci':'Offline');addEventListener('online',online);addEventListener('offline',offline);
 ...
 const regenerate=async item=>{if(!confirm(`Vytvořit nový přihlašovací odkaz pro ${item.name}? Starý odkaz přestane fungovat.`))return;try{setInviteCard(await request(`/admin/users/${encodeURIComponent(item.id)}/regenerate-code`,{method:'POST'}));}catch(e){setAdminError(e.message);}};
 ...
 const copyInvite=async()=>{await navigator.clipboard.writeText(currentInviteLink);alert('Přihlašovací odkaz byl zkopírován.');};
 const shareInvite=async()=>{if(!inviteCard)return;const text=`Ahoj, otevři tento odkaz pro automatické přihlášení do DFM:\n${currentInviteLink}`;if(navigator.share)await navigator.share({title:'Pozvánka do DFM',text,url:currentInviteLink});else{await navigator.clipboard.writeText(text);alert('Pozvánka byla zkopírována.');}};
 ...
 if(auth==='login')return <div className="auth-screen"><section className="auth-card"><div className="auth-logo">DFM</div><p className="auth-kicker">Drone Fleet Manager</p><h1>{initialized?'Přihlášení':'První spuštění'}</h1><form onSubmit={login}>{!initialized&&<label>Jméno<input value={name} onChange={e=>setName(e.target.value)} required/></label>}<label>Firemní e-mail<input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="jmeno@dronetech.cz" required/></label><label>{initialized?'Přístupový kód':'Startovací kód'}<input className="pin-input" value={code} onChange={e=>setCode(e.target.value.toUpperCase())} placeholder="ABCD-EFGH" required/></label>{error&&<p className="auth-error">{error}</p>}<button disabled={busy}>{busy?'Přihlašuji…':initialized?'Přihlásit se':'Vytvořit administrátora'}</button></form></section></div>;
