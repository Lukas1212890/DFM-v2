  ...
  Storage.prototype.setItem=function(key,value){
    if(this===localStorage&&key===STORAGE_KEY){
      try{
        ...
        const next=normalizeArchive(previous,JSON.parse(value));
...
function hideArchivedInActiveTab(){
  ...
  const lists=[...content.querySelectorAll(':scope > .list')];
...
function writeAndReload(data){
  localStorage.setItem(STORAGE_KEY,JSON.stringify(data));
  setTimeout(()=>location.reload(),120);
}

function restoreRecord(droneId,type,id){
  const data=readData(),drone=(data.drones||[]).find(item=>item.id===droneId),meta=TYPES[type];
...
function renderArchive(){
  ...
  const tabs=document.querySelector('.detail-hero')?.parentElement?.querySelector('.tabs');
  if(!drone||!tabs)return;
  [...tabs.parentElement.children].forEach(node=>{if(node!==tabs&&node.classList.contains('drone-archive-panel')===false&&node.compareDocumentPosition(tabs)&Node.DOCUMENT_POSITION_FOLLOWING)node.hidden=true});
  ...
  panel.hidden=false;
  panel.innerHTML=`<div class="archive-head"><div><p class="eyebrow">Historie dronu</p><h2>Archiv · ${items.length}</h2></div><div class="archive-summary"><span>🚨 ${(drone.accidents||[]).length}</span><span>⚠️ ${(drone.claims||[]).length}</span><span>🔧 ${(drone.services||[]).length}</span></div></div><div class="archive-filter"><button class="active" data-filter="all">Vše</button><button data-filter="accident">🚨 Nehody</button><button data-filter="claim">⚠️ Reklamace</button><button data-filter="service">🔧 Servis</button></div><div class="archive-list">${items.map(({type,item})=>`<article class="archive-card" data-type="${type}"><div class="archive-icon">${TYPES[type].icon}</div><div><small>${TYPES[type].tab.slice(0,-1)} · ${recordDate(item)}</small><h3>${recordTitle(item)}</h3><p>${item.description||item.technician||item.pilot||'Bez popisu'}</p><em>Uzavřeno: ${item.closedAt?new Date(item.closedAt).toLocaleDateString('cs-CZ'):'neuvedeno'}</em></div><div class="archive-actions"><button data-restore="${type}:${item.id}">↩ Obnovit</button>${isAdmin?`<button class="danger" data-delete="${type}:${item.id}">Smazat</button>`:''}</div></article>`).join('')||'<div class="empty">Archiv je zatím prázdný.</div>'}</div>`;
  panel.querySelectorAll('.archive-filter button').forEach(button=>button.onclick=()=>{
    panel.querySelectorAll('.archive-filter button').forEach(item=>item.classList.toggle('active',item===button));
    panel.querySelectorAll('.archive-card').forEach(card=>card.hidden=button.dataset.filter!=='all'&&card.dataset.type!==button.dataset.filter);
  });
  panel.querySelectorAll('[data-restore]').forEach(button=>button.onclick=()=>{const[type,id]=button.dataset.restore.split(':');restoreRecord(drone.id,type,id)});
  panel.querySelectorAll('[data-delete]').forEach(button=>button.onclick=()=>{const[type,id]=button.dataset.delete.split(':');deleteRecord(drone.id,type,id)});
}
...
function enhanceDetail(){
  ...
  if(!archive){
    archive=document.createElement('button');archive.type='button';archive.className='archive-tab';archive.textContent='Archiv';archive.onclick=()=>{
      [...tabs.querySelectorAll('button')].forEach(button=>button.classList.toggle('active',button===archive));
      renderArchive();
