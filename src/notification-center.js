...
function openCenter(){
  ...
  items.forEach(item=>{const button=document.createElement('button'),icon=document.createElement('span'),content=document.createElement('div'),title=document.createElement('strong'),detail=document.createElement('small'),arrow=document.createElement('b'),seen=read.has(item.key);button.type='button';button.className=`notification-item ${item.type}${seen?' read':''}`;icon.textContent=item.icon;title.textContent=item.title;detail.textContent=seen?`${item.detail} · zobrazeno`:item.detail;arrow.textContent='›';content.append(title,detail);button.append(icon,content,arrow);button.addEventListener('click',()=>{const keys=readKeys();keys.add(item.key);saveKeys(keys);button.classList.add('read');updateNotificationBell();navigateTo(item.section)});list.appendChild(button);});
