...
async function notifyFlightChanges(env,previousState,nextState){
  ...
  for(const flight of nextState?.flights||[]){
    const old=before.get(flight.id),pilot=pilots.get(flight.pilotId),assignmentChanged=!old||JSON.stringify({u:old.assignedUserId||old.assignedEmail||old.pilotId,d:old.date,l:old.location,r:old.droneId})!==JSON.stringify({u:flight.assignedUserId||flight.assignedEmail||flight.pilotId,d:flight.date,l:flight.location,r:flight.droneId});
    ...
    if(assignmentChanged){const identity=String(flight.assignedUserId||flight.assignedEmail||pilot?.appUserId||pilot?.email||pilot?.name||'').trim().toLowerCase();if(identity){const user=await env.DB.prepare('SELECT id FROM users WHERE active=1 AND (lower(id)=? OR lower(email)=? OR lower(name)=?)').bind(identity,identity,identity).first();if(user)await sendToUser(env,user.id,{title:old?'Naplánovaný let byl upraven':'Máte naplánovaný nový let',body:details||'Otevřete DFM pro podrobnosti.',tag:`flight-${flight.id}`,url:'./#flights'});}}
    if(flight.acceptedAt&&old?.acceptedAt!==flight.acceptedAt){for(const adminId of adminIds)await sendToUser(env,adminId,{title:`Let přijal ${flight.acceptedByName||flight.assignedName||pilot?.name||'pilot'}`,body:details||'Pilot potvrdil přijetí letu.',tag:`flight-accepted-${flight.id}`,url:'./#flights'});}
    if(flight.completedAt&&old?.completedAt!==flight.completedAt){for(const adminId of adminIds)await sendToUser(env,adminId,{title:`Let dokončil ${flight.completedByName||flight.acceptedByName||flight.assignedName||pilot?.name||'pilot'}`,body:details||'Pilot označil let jako hotový.',tag:`flight-completed-${flight.id}`,url:'./#flights'});}
  }
}
